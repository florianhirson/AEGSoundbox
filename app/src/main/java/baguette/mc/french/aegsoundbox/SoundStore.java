...
public abstract class SoundStore {
    ...
    public static ArrayList<Sound> getAllSounds(Context context) {
        ...
        TypedArray pictureIds = res.obtainTypedArray(R.array.pictureIds);

        ArrayList<Sound> sounds = new ArrayList<>();

        for (int i = 0; i < labels.length(); i++) {
            sounds.add(new Sound(labels.getString(i), ids.getResourceId(i, -1), pictureIds.getResourceId(i, -1)));
        }
    ...
    public static ArrayList<Sound> getFavoriteSounds(Context context) {
        ...
        for (int i = 0; i < labels.length(); i++) {
            Sound sound = new Sound(labels.getString(i), ids.getResourceId(i, -1), pictureIds.getResourceId(i, -1));
